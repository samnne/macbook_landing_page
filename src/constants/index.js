...
// Apply `asset` to known asset paths so consumers can use them directly
const performanceImagesWithAsset = performanceImages.map((p) => ({
  ...p,
  src: asset(p.src),
}));

const featuresWithAsset = features.map((f) => ({
  ...f,
  icon: asset(f.icon),
}));

const featureSequenceWithAsset = featureSequence.map((s) => ({
  ...s,
  videoPath: asset(s.videoPath),
}));

export {
  featuresWithAsset as features,
  featureSequenceWithAsset as featureSequence,
  footerLinks,
  navLinks,
  noChangeParts,
  performanceImagesWithAsset as performanceImages,
